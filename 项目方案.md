...
每天定时抓取 AI 新闻，Claude 整理成中文简报，配图+卡片格式，自动发到你的飞书群聊。完全跑在 GitHub 的免费云服务器上，不需要你自己开电脑。

## 功能亮点

- 🌐 **多源抓取**：从 6 个主流 AI 资讯 RSS 源抓取
- 🇨🇳 **中文摘要**：Claude 自动翻译总结，非机翻，用自己的话概括
- 🖼️ **自动配图**：从原文提取头图，飞书卡片内展示
- 🎴 **卡片消息**：飞书交互式卡片格式，排版精美
- 🕗 **定时全自动**：每天北京时间 8 点自动跑
...
| 文件 | 说明 |
|------|------|
| `fetch_ai_news.py` | 主脚本：抓 RSS → 提取配图 → Claude 中文摘要 → 飞书卡片 |
...
```mermaid
flowchart LR
    A[🕗 定时触发<br/>每天北京时间 8:00] --> B[📡 抓取 RSS 源<br/>提取标题+链接+配图]
    B --> C{🈶 有 ANTHROPIC_API_KEY?}
    C -->|有| D[🤖 Claude AI 中文摘要<br/>逐条翻译+今日看点]
    C -->|没有| E[📋 降级模式<br/>展示原文摘要]
    D --> F[🎴 生成飞书卡片<br/>标题+摘要+配图]
    E --> F
    F --> G[📨 飞书 Webhook<br/>推送到群聊]
    G --> H[✅ 群聊收到精美日报]
```
